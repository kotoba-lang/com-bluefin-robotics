(ns bluefin-robotics.main
  "Kotodama WASM entrypoint for the Bluefin_robotics clean-room actor (L4) — Clojure port.
  ...
  facts (`bluefin_robotics.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bluefin_robotics")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"bluefin_robotics.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bluefin_robotics-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bluefin_robotics-compat/ready)
